...
@section('content_header')
    <span><a href="{{route('admin.product.index',$cat->id)}}"> Danh mục: {{$cat->ten_danh_muc}} </a> / Cập nhật sản phẩm</span>
@stop
@section('content')
    @if(count($errors) > 0)
    ...
    <div class="callout-top callout-top-danger pt-0">
        <form action="{{route('admin.product.update', $data->id)}}" enctype="multipart/form-data" method="POST">
            ...
            <div class="form-group mt-2">
                ...
                <input type="text" name="ten_san_pham" value="{{$data->ten_san_pham}}" class="form-control"
                       aria-describedby="cat" required>
            </div>
            <div class="form-group">
                ...
                @isset($data->hinh_anh)
                    <div>
                        <img style="width: 150px;height: 100px" src="/uploads/product/{{$data->hinh_anh}}" alt="ảnh">
                    </div>
                @endisset
            </div>
            <div class="form-group">
            ...
            <div class="row">
                ...
                <div class="form-group col-md-4 col-12">
                    ...
                    <input type="number" name="giam_gia" value="{{$data->giam_gia}}" class="form-control"
                           aria-describedby="cat" required>
                </div>
                <div class="form-group col-md-4 col-12 d-none">
                    <label for="cat">Danh mục:</label>
                    <input type="text" name="loai_san_pham_id" value="{{$cat->id}}" class="form-control"
                           aria-describedby="cat">
                </div>
            </div>
